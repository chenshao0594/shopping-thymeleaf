...
function miniCart(cartCode){
	console.log('Display cart content');
	$.ajax({  
		type: 'GET',  
		url: '/cart/miniCart?cartCode='+cartCode,  
		cache:false,
		error: function(e) { 
			console.log('error ' + e);
		},
		success: function(miniCart) {
			console.log('Success ' + miniCart.code);
		//	cleanupMiniCart();
...
function displayMiniCartSummary(code){
	$.ajax({  
		type: 'GET',  
		url:'/shop/cart/miniCart?cartCode='+code,  
		error: function(e) { 
...
/*	var miniCartSummaryTemplate = Hogan.compile(document.getElementById("miniCartSummaryTemplate").innerHTML);
	...
	$('#mini-cart').html(miniCartSummaryRendered);
*/
}
